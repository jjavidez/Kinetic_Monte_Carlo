...
subroutine vectors (r_vector, w_vector, r_prob)
    integer, intent(out) :: r_vector(2, l_box**2 - 1) ! relative position vectors to the first site
    real(8), intent(out) :: w_vector(l_box**2 - 1) ! weights for each vector
    real(8), intent(out) :: r_prob(l_box**2 - 1) ! probabilities for each vector
    real(8) :: weight_sum
